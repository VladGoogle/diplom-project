  ...
  return (
    <div className="container">
      {productInfo && ( // Check if productInfo exists and has a name
        <h1 className="reviews__title">Reviews on {productInfo.name}</h1>
      )}
      <section className="reviews">
        {loggedIn && (
          <div className="reviews__send-box">
            <h2 className="reviews__send-box--title">Leave your review</h2>
            <button
              onClick={() => setReviewFormOpen(true)}
              className="reviews__send-box--button"
            >
              REVIEW
            </button>
          </div>
        ...
        {reviewFormOpen && (
          <ReviewForm onCloseReview={() => setReviewFormOpen(false)} />
        )}
      </section>
    </div>
  );

...
const Order = ({
  ...
  const handleRefund = async () => {
    try {
      await instance.patch(`/account/order/${orderId}/refund`);
      // Обработка успешного возврата
      console.log('Refund successful');
    } catch (error) {
      // Обработка ошибки при возврате
      ...
      <div className="orders__top">
        <div className="orders__tab-left">
          <img
            src={status}
            alt="order status"
            className="orders__tab-status--image"
          />
          <div className="orders__tab-status--info">
            <span className="order__tab-status--id">{formattedDate}</span>
            <h3 className="orders__tab-status--title">In progress</h3>
          </div>
        </div>
        {!showContent && (
          <div className="orders__tab-middle">
            <span className="orders__tab-price--title">Order price</span>
            <h3 className="orders__tab-price">{total}$</h3>
          </div>
        )}
        ...
        <div className="orders__content">
          <div className="orders__content-left">
            <div className="delivery__info">
              ...
              <p className="delivery__company">Nova Poshta</p>
              <p className="delivery__pickup">
                Pickup from {address + ', ' + city}
              </p>
              <div className="delivery__schedule">
                <h4 className="delivery__schedule-title">Schedule:</h4>
          ...
          <div className="orders__content-right">
            <ul className="orders__products">
              ...
              {product.map((item, id) => (
                <li key={id} className="orders__product">
                  ...
                  <a className="orders__product-link">{item.product.name}</a>
                  <div className="orders__product-price">
                    <h4 className="orders__product-price--title">Price</h4>
            ...
            <div className="orders__payment">
              <h4 className="orders__payment--title">Payment</h4>
            ...
            <div className="orders__status">
              <h4 className="orders__status-title">Status</h4>
              <span className="orders__status-text">Paid</span>
            </div>
            <div className="orders__delivery">
              <h4 className="orders__delivery-title">Delivery</h4>
              <span className="orders__delivery-text">10 $</span>
            </div>
            <div className="orders__total">
              <h4 className="orders__total-title">Total</h4>
              <span className="orders__total-text">{total}$</span>
            </div>
            <button onClick={handleRefund} className="orders__button">
              REFUND
            </button>
          </div>
        </div>
